...
public class VRPlaneDestroy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        S.Start();
        OriginCube = GameObject.FindGameObjectWithTag("Destroyer");
        XCube = GameObject.FindGameObjectWithTag("XCube");
        YCube = GameObject.FindGameObjectWithTag("YCube");
        ...
        S.Stop();
        UnityEngine.Debug.Log(string.Format("Planar Setup on {0} took {1} ms to complete", gameObject.name, S.ElapsedMilliseconds));
    }
    ...
    {
        time += Time.deltaTime;
        UnityEngine.Debug.DrawLine(OriginCube.transform.position, XCube.transform.position, Color.red, 1f, false);
        UnityEngine.Debug.DrawLine(OriginCube.transform.position, YCube.transform.position, Color.blue, 1f, false);
        ...
        if (Controller.TryGetFeatureValue(CommonUsages.primaryButton,out bool primaryButtonValue))
        {
            ...
            {
                UnityEngine.Debug.Log("YOU PRESSED CONTROLLER BUTTON!");
                if (time > MaxTimer)
                {
                    Stopwatch S = new Stopwatch();
                    S.Start();
                    ...
                    time = 0f;
                    S.Stop();
                    UnityEngine.Debug.Log(string.Format("Planar Destruction on {0} took {1} ms to complete", gameObject.name, S.ElapsedMilliseconds));
                }   
            }
        }
        // Mouse Trigger for testing.
    ...
    private void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            bounds.Expand(0.5f);
            //var plane2 = new Pane(XCube.transform.position, YCube.transform.position, OriginCube.transform.position);
                
            Vector3 DestroyerToXVector = XCube.transform.position - OriginCube.transform.position;
            Vector3 DestroyerToYVector = YCube.transform.position - OriginCube.transform.position;
            Vector3 PlaneNormal = Vector3.Cross(DestroyerToYVector, DestroyerToXVector);
            //Vector3 positionOfPlaneInLocal = OriginCube.transform.position;
            //Vector3 scaleOfOrigin = OriginCube.transform.localScale;

            // Test Me
            float xPos = OriginCube.transform.position.x * OriginCube.transform.localScale.x;
            float yPos = OriginCube.transform.position.y * OriginCube.transform.localScale.y;
            float zPos = OriginCube.transform.position.z * OriginCube.transform.localScale.z;
            
            var plane = new Plane(PlaneNormal, new Vector3(xPos, yPos, zPos));
            UnityEngine.Debug.DrawLine(OriginCube.transform.position, XCube.transform.position, Color.red, 1000f, false);
            UnityEngine.Debug.DrawLine(OriginCube.transform.position, YCube.transform.position, Color.blue, 1000f, false);
            UnityEngine.Debug.DrawLine(new Vector3(0,0,0), PlaneNormal * 25, Color.cyan, 1000f, false);
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(this, useGravity);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(VRPlaneDestroy original, bool useGravity)
        {
            ...
            var meshDestroy = GameObject.AddComponent<VRPlaneDestroy>();
